...
**Export:** `GET /api/Mediation/MediationContract/export` — includes `Total Cost`, `Paid`, `Remaining`, `Payment Status`.

**List/detail financial fields (`MediationContractListDto` / `MediationContractDto`)** — backend-calculated; do **not** recompute in UI:

| Field | Type | Notes |
|-------|------|-------|
| `totalCost` | decimal? | Contract total |
| `totalPaid` | decimal | Sum of completed payments |
| `remainingAmount` | decimal | `max(0, totalCost - totalPaid)` |
| `paymentPercentage` | decimal | 0–100 |
| `paymentStatus` | string | Arabic: `غير مدفوع` / `مدفوع جزئياً` / `مدفوع بالكامل` |
| `paymentStatusCode` | enum | `Unpaid` / `PartiallyPaid` / `Paid` |
| `currency` | string | Always `SAR` — format as `3,499.41 SAR` client-side |

**Details-only:**

| Field | Type |
|-------|------|
| `payments[]` | Payment history (`paymentDate`, `amount`, `paymentMethod`, `referenceNumber`, `notes`, `createdBy`) |
| `financialSummary` | `{ offerAmount, vat, otherCosts, salary, totalContractValue, totalPaid, remainingAmount, currency }` |

**Record payment:** `POST /api/Mediation/MediationContract/customer-payment`
...
```json
{
  ...
  "paymentMethod": 2,
  "referenceNumber": "INV-1542",
  "notes": "Bank transfer",
  "bankFees": 0
}
```

Response includes updated `totalPaid`, `remainingAmount`, `paymentPercentage`, `paymentStatus`, `financialSummary`, and `payments`.

**Paid filters:** `isPaid=true` → fully paid by amount; `isUnpaid=true` → no completed payments; `paymentDateFrom` / `paymentDateTo` filter on payment rows (not Musaned date).

See also: `Mediation_Financial_Calculations.md` §4.
...
- [ ] Mediation list: show `totalPaid`, `remainingAmount`, `paymentStatus`; details: progress bar + payment history + financial summary (all from API)
- [ ] Mediation list: support new filters + `page` pagination param + paid/unpaid/payment-date filters
