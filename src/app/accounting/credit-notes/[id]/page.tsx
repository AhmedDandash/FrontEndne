...
/**
 * Credit note detail route — Phase 2 of the modal→route migration (the 4
 * accounting documents), mirroring Phase 1's contract routes. Renders the
 * same `CreditNoteDetailView` the list page's detail drawer used to show;
...
import { useCreditNote, useCreditNoteTrace } from '@/hooks/api/useCreditNotes';
import RecordDetailShell from '@/components/record-detail/RecordDetailShell';
import CreditNoteDetailView from '../_components/CreditNoteDetailView';
import { DocumentTraceDrawer } from '../../_lib/DocumentTraceDrawer';

const LIST_ROUTE = '/accounting/credit-notes';
...
export default function CreditNoteDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: note, isLoading, isError, error, refetch } = useCreditNote(id);

  const [traceOpen, setTraceOpen] = useState(false);
  const { data: traceData, isLoading: isTraceLoading } = useCreditNoteTrace(traceOpen ? id : undefined);

  const notFound = isError && isNotFoundError(error);
  const genericError = isError && !notFound;

  const displayNumber = note?.creditNoteNumber || `#${id}`;
  ...
  return (
    <>
      <RecordDetailShell
        ...
        onRetry={() => refetch()}
        breadcrumbs={[
          { label: t('إشعارات الدائن', 'Credit Notes'), href: LIST_ROUTE },
          { label: displayNumber },
        ]}
        backHref={LIST_ROUTE}
        title={displayNumber}
      ...
      >
        {note && <CreditNoteDetailView note={note} isAr={isAr} />}
      </RecordDetailShell>
